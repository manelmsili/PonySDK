...
public class UISampleEntryPoint implements EntryPoint, UserLoggedOutHandler {
	...
	private void testWebSocket() {
		...
		// final PLabel ConnectLabel1 = Element.newPLabel("Click here to connect
		// via PWS1");
		...
		// final PWebSocket pWebSocket1 = new
		// PWebSocket("wss://mmsili.smart-trade.net:8082/sample/chat/1");
		// pWebSocket1.attach(PWindow.getMain(), null);
		// });
		//
		// final PLabel ConnectLabel2 = Element.newPLabel("Click here to connect
		// via PWS2");
		...
		// final PWebSocket pWebSocket2 = new
		// PWebSocket("wss://mmsili.smart-trade.net:8082/sample/chat/2");
		// pWebSocket2.attach(PWindow.getMain(), null);
		// });
		//
		// final PLabel sendAudioLabel = Element.newPLabel("Click here to send
		// SOUND ! ");
		...
		final int remoteUrl = i == 1 ? 2 : 1;

		final PLabel ConnectLabel = Element.newPLabel("Click here to connect via PRTC (init)" + i);
		panel.setAttribute("align", "center");
		ConnectLabel.addStyleName("connect-label");
		ConnectLabel.addStyleName("red");
		panel.setAttribute("font-size", "15px ");
		panel.add(ConnectLabel);
		PWindow.getMain().add(panel);
		final PRTCConnection pRTCConnection = new PRTCConnection("wss://mmsili.smart-trade.net:8082/sample/chat/" + i);
		pRTCConnection.setRemoteURL("wss://mmsili.smart-trade.net:8082/sample/chat/" + remoteUrl);
		ConnectLabel.addClickHandler(event -> {
			pRTCConnection.attach(PWindow.getMain(), null);
		});

		final PLabel sendAudioLabel = Element.newPLabel("Click here to send SOUND ! (start)");
		sendAudioLabel.addStyleName("pLabel");
		sendAudioLabel.addStyleName("blue");
		sendAudioLabel.addClickHandler(event -> pRTCConnection.start());

		panel.add(sendAudioLabel);
		PWindow.getMain().add(panel);
		// faire un cadre pour les deux videos
		final PElement audioElement = Element.newPElement("audio");
		audioElement.setAttribute("id", "audioId");
		// audioElement.setAttribute("src");
		audioElement.setAttribute("font-size", "15px");
		audioElement.setAttribute("controls", "controls");
		audioElement.setAttribute("autoplay", "autoplay");
		audioElement.addStyleName("audioLabel");
		panel.add(audioElement);

		// final AudioElement audio = window.newAudioElement("audio");
		// audio.setId("myAudio");
		// audio.setTextContent("bouton audio");
		// audio.setAttribute("align", "center");
